...
1. Initializes an EEPROM block of size 64 bytes in the internal flash of the XMC MCU
...
The main function only calls the `E_EEPROM_example()` function which consists of two steps as follows:
...
- **Step 2**: Initializes the emulated EEPROM application using the `E_EEPROM_XMC4_Init` function for the XMC4700 Relax Kit and the `E_EEPROM_XMC1_Init()` function for the XMC1400 Boot Kit and copies the latest saved data from the flash to the reserved RAM buffer.

- **Step 3**: Reads the data from an EEPROM block. Afterward, it increments the read data by 1 and programs it back to the EEPROM. Finally, it reads and prints the new data from the EEPROM. For the XMC1400 Boot Kit, all the substeps are performed inside the `E_EEPROM_test()` function called from the `E_EEPROM_example()` function. For the XMC4700 Relax Kit, all the substeps are programmed inside the `E_EEPROM_example()` function.
